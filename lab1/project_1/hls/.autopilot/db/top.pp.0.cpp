...
__attribute__((sdx_kernel("top_kernel", 0))) void top_kernel(data_t A[256][64],
                data_t C[256][64]) {
#line 19 "/nethome/wsun377/ece8893/FPGA_ECE8893/2026_Spring/lab1/script.tcl"
...
# 6 "top.cpp"
    ...
    VITIS_LOOP_11_1: for (int i = 0; i < 256; i++) {
        ...
        VITIS_LOOP_23_3: for (int j = 0; j < 64; j++) {
            tmp[i][j] = A[i][j] / denom;
        }
    }


    VITIS_LOOP_29_4: for (int j = 0; j < 64; j++) {
        data_t col_sum = 0.0;


        VITIS_LOOP_33_5: for (int i = 0; i < 256; i++) {
            col_sum += tmp[i][j];
        }
        ...
        VITIS_LOOP_41_6: for (int i = 0; i < 256; i++) {
            C[i][j] = tmp[i][j] * scale;
        }
    }
}
