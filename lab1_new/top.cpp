...
// - Removes the extra on-chip C buffer + copy-back loop by writing results directly to C_DRAM.
...
#pragma HLS UNROLL
    ...
    // Pass 4: direct write-back to C_DRAM (removes on-chip C + copy-back loop)
    // This stage is now mostly I/O bound (one AXI write per cycle).
    for (int i = 0; i < N_ROWS; i++) {
        for (int j = 0; j < N_COLS; j++) {
#pragma HLS PIPELINE II=1
